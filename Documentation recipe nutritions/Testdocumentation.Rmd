...
To obtain the final dataset, we carried out several pre-processing steps. First, we collected all recipes from each diet separately. Then, for each diet, we collected the recipes we were interested in with respect to dinner and dish type. Finally, a loop was created tabulating diet type, recipe name, cuisine type and amounts of carbohydrates, fats and proteins. Labels were created for meal type, dish type and cuisine type. There were no missing values in the final data set, so no missing values were removed. As mentioned, calorie counts did not seem reliable, hence they were not included in the dataset. The time for the recipe has not been added to the dataset, because the main focus for this dataset lays on the macronutrients and for users to see how many grams they consume if they make a certain recipe.

--> mactornutrients nient meegenoemn toevoegen
...
There is no repository available yet for papers and systems that use the data scraped in this project. However, we have our repository that contains all the files and the dataset. The link is: https://github.com/Mandanakhabbazi/recipe-nutritions.git
...
The dataset should in no way be used to guide nutrition. It should only be used for research purposes.
→ do not use it as the only guideline for nutrition (you also have vitamins etc.)
