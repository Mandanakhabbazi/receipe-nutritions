...
34,898 instances have been collected for the raw data. The data have been saved into a .json output to be further processed. The code in the repository creates a dataset with for every recipe the health label, meal type, dish type, cuisine type and the accompanying macro-nutrients. The macro-nutrients are given in integer variables, this is so users are able to calculate with the numbers. The following tables give an explicit overview of the variables used in the code. 

--> TABBELLEN MOETEN NOG INGEVOEGD WORDEN
tab_nutrrientstructure <- matrix(c('String', 'String', 'Float', 'String'), ncol=2, byrow = TRUE)
colnames(tab_nutrrientstructure) <- c('Data type', 'Description')
rownames(tab_nutrrientstructure) <- c('uri', 'label', 'quantity', 'unit')


rownames(tab_nutrrientstructure) <- ('Ontology identifier', 'Display label', 'Quantity of specific units', 'Units')
tab_nutrrientstructure <- as.table(tab_nutrrientstructure)
tab_nutrrientstructure
...
The .json file contains all the collected data. With this dataset users are able to conduct their own dataset with the data they like to use. The NAAM file contains a dataset produced by the makers of this project to see how many grams each macronutrients contains for recipes. A recommended data split could be based on the quantity for the macronutrients based on specific diets users are following. Another split could be based on certain cuisine types users prefer. Other splits are also possible, the splits stated above are however recommended by the makers of the database with the thought in mind for the purpose of this dataset. 
...
This dataset does not contain data that one could consider confidential. This can be assumed because the data is publicly available on the Edamam API website. There is no information in the data that could trace back to private  information (personal data, locations etc.)
...
The data associated with each instance is acquired through the API from Edamam. Using the script for the (HIER HET PATH IN ZETTEN)  .json file of all the different diets, the data of the recipes with all the macronutrients can be viewed. To make it more clear, a table is provided for every diet in which the data of the different recipes can also be observed. 
...
Only the students of this project were involved in the data collection process. There was no compensation because this was not applicable to this project. 
...
The data collected for the project was collected in real time. The end and total dataset has been collected on XXXX. --> DATUM TOEVOEGEN
...
The dataset does not relate to people, therefore, the questions in the following section are not applicable. 
