...
The dataset is a sample from a larger set obtained through deterministic sampling. Based on the site of (PlanKetogenic, z.d.) which contains the most researched diets worldwide, the most popular diets in the countries who use Edamam the most, were chosen and filtered for the dataset. For each diet the dataset is filtered on the macronutrients fats, carbs and proteins to be able to see how many grams each recipe contains. The data set is also filtered on lunch/dinner, the reason behind this filter is because intermediate fasting (in which a person consumes their food between 12pm and 8am) has become very popular in the last few years (Mohiuddin, 2018; Snyder, 2022). This means people more often are choosing to skip breakfast, so lunch/dinner seemed like a more useful meal type to extract information from. The cuisine type is also added in order to see from which cuisine the recipe originates. The dataset has been filtered for the first 8000 diets because the programs used (Jupyter Notebook and Anaconda) were not able to run the script otherwise due to license issues. Unfortunately, this makes it not possible to automate the script. The scraper however provides comments of what to do to get the csv file in which the nutrition information of all diets are combined. This also makes it easier for future users of the scraper, because there is a guide on how to add the diets to the scraper one is interested in. 
...
The raw data have been saved in multiple files, according to the diets. The files are: 'keto_raw_data.json', 'dash_raw_data.json', 'vegan_raw_data.json', 'paleo_raw_data.json', and 'mediterranean_raw_data.json'.
